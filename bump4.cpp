                             ...
                             bool allow_skb_fallback = true)
{
    ...
    std::fprintf(stderr, "[%s] xsk_socket__create failed: ret=%d (%s)\n",
...
// -----------------------------------------------------------------------------
// Worker thread: poll + forward in one direction
// -----------------------------------------------------------------------------
...
int main(int argc, char** argv) {
    ...
    std::thread t_ab(forward_worker, "A→B",
                     &epA, &umemA,
                     &epB, &umemB,
                     &g_running, verbose);

    std::thread t_ba(forward_worker, "B→A",
                     &epB, &umemB,
                     &epA, &umemA,
                     &g_running, verbose);
