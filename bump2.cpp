    ...
    if (debug)
    {
            for (unsigned int i = 0; i < rcvd; ++i) {
                ...
                if (parse_ethertype_vlan_aware(frame, rx_lens[i], et)) {
                    std::fprintf(stderr, "[RXx] len=%u ethertype=0x%04x (%s)\n",
                                rx_lens[i], et, ethertype_to_str(et));
                } else {
                    std::fprintf(stderr, "[RXa] len=%u ethertype=? (frame too short/malformed)\n",
                                rx_lens[i]);
                }
            }
    }
...
int main(int argc, char** argv) {
    ...
    // Fill ring gets (frame_count - TX_RESERVE) frames; the rest become TX freelist.
    ...
    std::cout << "Sockets created and rings mapped. Starting bidirectional forwarding...\n";

    // Poll both sockets and forward both directions
    struct pollfd fds[2];
    fds[0].fd = epA.fd; fds[0].events = POLLIN;
    fds[1].fd = epB.fd; fds[1].events = POLLIN;

    while (true) {
        int pr = poll(fds, 2, 100 /*ms*/);
        if (pr < 0) {
            perror("poll");
            break;
        }
        // Forward A->B
        forward_step(epA, umemA, epB, umemB);
        // Forward B->A
        forward_step(epB, umemB, epA, umemA,true);
    }
