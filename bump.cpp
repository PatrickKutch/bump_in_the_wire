...
struct UmemArea {
    ...
    uint64_t            next_frame = 0;     // next frame index to hand out
    ...
    // Free-list for user-managed TX frames (used by the TX endpoint's UMEM)
...
static inline void init_free_frames(UmemArea& ua) {
    ua.free_frames.clear();
    ua.free_frames.reserve(ua.frame_count);
    for (uint32_t i = 0; i < ua.frame_count; ++i) {
...
static inline bool parse_ethertype_vlan_aware(const uint8_t* frame, uint32_t len, uint16_t& ether_type_out) {
    // Ethernet header must be at least 14 bytes
    if (len < 14) return false;
    size_t off = 12;
    uint16_t et = 0;
    if (!read_be16(frame + off, et)) return false;

    // Walk VLAN tags (up to 2 levels) to find the actual EtherType
    int max_tags = 2;
    while ((et == 0x8100 || et == 0x88A8 || et == 0x9100) && max_tags-- > 0) {
        // VLAN header is 4 bytes: TCI (2) + encapsulated EtherType (2)
               ...
               uint32_t comp_size = 4096)
{
    ...
    // AF_XDP UMEM should be page-aligned; use posix_memalign
    ...
    ucfg.flags          = 0; // default

    ret = xsk_umem__create(&ua.umem,
                           ua.buffer,
                           ua.size,
                           &ua.fill,
                           &ua.comp,
...
int setup_rings(const char* ifname,
                ...
                XskEndpoint& ep)
{
    int ret = xsk_socket__create(&ep.xsk,
                                 ifname,
                                 queue_id,
                                 ua.umem,
                                 &ep.rx,
...
// Prefill the UMEM fill ring with frame addresses, in chunks.
// Returns number of frames queued.
uint32_t prefill_fill_ring(UmemArea& ua)
{
    // Prefill in modest chunks to avoid over-asking the ring.
    const uint32_t want  = ua.frame_count;
    const uint32_t CHUNK = 256; // safe batch size for reservation
    uint32_t total = 0;
    ...
    while (total < want) {
        ...
        if (space == 0) {
            // Ring full for now; stop prefill
            break;
        }
        for (uint32_t i = 0; i < space; ++i) {
            // AF_XDP addresses are offsets into UMEM.
...
// -----------------------------
void forward_step(XskEndpoint& in_ep, UmemArea& in_umem,
                  XskEndpoint& out_ep, UmemArea& out_umem)
{
    ...
    // ----- DEBUG: print EtherType and length for each packet -----
    for (unsigned int i = 0; i < rcvd; ++i) {
        const uint8_t* frame = umem_ptr(in_umem, rx_addrs[i]);
        uint16_t et = 0;
        if (parse_ethertype_vlan_aware(frame, rx_lens[i], et)) {
            std::fprintf(stderr, "[RX] len=%u ethertype=0x%04x (%s)\n",
                         rx_lens[i], et, ethertype_to_str(et));
        } else {
            std::fprintf(stderr, "[RX] len=%u ethertype=? (frame too short or malformed)\n",
                         rx_lens[i]);
        }
    }
    // -------------------------------------------------------------
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <input_netdev> <output_netdev>\n";
        return 1;
    }
    ...
    std::cout << "Input device:  " << input_dev  << "\n";
    std::cout << "Output device: " << output_dev << "\n";
    ...
    cfg.bind_flags = 0; // copy mode (do not request ZEROCOPY here)

    // --- Setup UMEM for each endpoint (separate UMEMs) ---
    UmemArea in_umem {};
    UmemArea out_umem {};

    const uint32_t FRAME_COUNT = 8192; // tune as needed
    const uint32_t FRAME_SIZE  = 2048; // must hold your MTU
    const uint32_t HEADROOM    = 0;

    if (setup_umem(in_umem, FRAME_COUNT, FRAME_SIZE, HEADROOM)) {
        std::cerr << "Failed to setup UMEM for input\n";
        return 1;
    }
    if (setup_umem(out_umem, FRAME_COUNT, FRAME_SIZE, HEADROOM)) {
        std::cerr << "Failed to setup UMEM for output\n";
        xsk_umem__delete(in_umem.umem);
        free(in_umem.buffer);
        return 1;
    }
    ...
    if (create_xsk_socket(input_dev,  xsk_in,  in_umem,  cfg, /*queue_id=*/0)) {
        xsk_umem__delete(out_umem.umem);
        free(out_umem.buffer);
        xsk_umem__delete(in_umem.umem);
        free(in_umem.buffer);
        return 1;
    }
    if (create_xsk_socket(output_dev, xsk_out, out_umem, cfg, /*queue_id=*/0)) {
        xsk_socket__delete(xsk_in.xsk);
        xsk_umem__delete(out_umem.umem);
        free(out_umem.buffer);
        xsk_umem__delete(in_umem.umem);
        free(in_umem.buffer);
    ...
    // ✅ Prefill ONLY AFTER the input socket exists (so the kernel sees the rings)
    uint32_t in_prefilled  = prefill_fill_ring(in_umem);
    std::cout << "Prefilled input UMEM fill ring with "  << in_prefilled  << " frames\n";

    // ✅ For TX, keep output UMEM frames under our control
    init_free_frames(out_umem);
    std::cout << "Initialized " << out_umem.free_frames.size()
              << " free TX frames in output UMEM\n";

    std::cout << "Sockets created and rings mapped. Starting forwarding...\n";
    ...
    while (true) {
        // Wait up to 100ms for RX readiness; even on timeout, do a step (reap comps)
        int pr = poll(fds, 1, 100);
        if (pr < 0) {
            perror("poll");
            break;
        }
        forward_step(xsk_in, in_umem, xsk_out, out_umem);
    }

    // Cleanup (unreached in this loop, but kept for completeness)
    xsk_socket__delete(xsk_out.xsk);
    xsk_socket__delete(xsk_in.xsk);
    xsk_umem__delete(out_umem.umem);
    xsk_umem__delete(in_umem.umem);
    free(out_umem.buffer);
